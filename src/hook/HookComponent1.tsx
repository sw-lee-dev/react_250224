...
// Hook 함수의 종류
// - useState : 컴포넌트의 상태 관리를 가능하게 해주는 함수 (상태 생성, 상태 업데이트, 상태 접근)
...
// - useReducer : 상태와 관련된 로직을 관리하는 함수 (상태 업데이트 함수(-setXXX함수)를 컴포넌트 외부로 옮길 수 있도록 하여 코드 관리를 수월하게 도와줌)
// - useCallback : 메모리 상에서 함수를 재호출할 수 있도록 하는 함수 (렌더링 혹은 이벤트 없이 특정 함수를 호출할 때 사용)
// - useMemo : 메모리 내에 함수의 결과를 저장하여 함수를 재호출하지 않고 결과를 사용할 수 있도록 하는 함수

//!- Hook 함수는 반드시 함수형 컴포넌트 코드 블럭 라인에 존재해야함
// const [state, setState] = useState<number>(0); >> React Hooks must be called in a React function component
export default function HookComponent1() {
  ...
  // useEffect :
  // - 컴포넌트의 생명주기에 따라 특정 함수를 호출하는 Hook 함수
