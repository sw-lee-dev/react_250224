...
// 자식 컴포넌트로 상태 전달 :
// - 속성을 통해 부모 요소의 상태를 자식 요소로 전달할 수 있음 (공통되는 부모 요소를 각 자식 요소로 전달)
// !문제점 : 
// - 자식 컴포넌트에서 상태를 변경하면 부모 컴포넌트가 리렌더링 됨
// - 부모 컴포넌트가 리렌더링 되면서 해당 상태를 사용하지 않는 자식 컴포넌트까지 리렌더링 됨
// - 사용하지 않는 속성을 자식 컴포넌트에 넘겨주기 위해 받아오기 때문에 코드의 복잡성이 증가
// - 이 문제를 해결하기 위해서 글로벌 상태로 관리하는 방법이 파생됨
//?- 글로벌 상태 관리 방법으로는 context(임의의 공간), Redux(라이브러리), zustand라는 기법이 존재
