...
// Properties (속성)
// - 부모 컴포넌트(호출부 위치)에서 자식 컴포넌트로 데이터를 전달하기 위한 *객체*
// - 부모 컴포넌트에서는 HTML과 동일한 방식으로(속성=값) 전달할 수 있음
// - 자식 컴포넌트에서는 함수의 매개변수로 속성을 받음
// - 전달할 수 있는 데이터는 변수에 담을 수 있는 모든 데이터 형식

// - **컴포넌트가 리렌더링되는 기준이 됨**(속성값이 바뀌면 리렌더링)
...
function Article({ title, subTitle, contents }: Props) {
  ...
  // const { title, subTitle, contents } = props; >> 객체 디스트럭처링
...
export default function Properties() {
  ...
  return (
    <div>
      ...
      {/* <Child getTitle={getTitle} children={<p>익숙한 속성...</p>} /> */}
